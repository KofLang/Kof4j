...
public final class StatementAnalyzer {
    ...
    static Type analyzeAssignmentStatement(SemanticAnalyzer sa, AssignmentExpr ae, SymbolTable scope) {
        ...
        // SG-005/008 (SEM048): `x = null` é erro se x não é nullable (T?)
        if (CompilerComparisons.isNullLiteral(ae.value()) && sa.diagnostics() != null) {
            Type prelimTarget = Type.UnknownType.UNKNOWN;
            if (ae.target() instanceof IdentifierExpr ie0) {
                SymbolTable.Symbol sym0 = scope.resolve(ie0.name());
                if (sym0 != null) prelimTarget = sym0.type();
            }
            boolean targetNullable = !Type.isUnknown(prelimTarget) && Type.display(prelimTarget).endsWith("?");
            if (!targetNullable) {
    ...
    static void analyzeStatement(SemanticAnalyzer sa, StatementNode stmt, SymbolTable scope, Type returnType) {
        switch (stmt) {
            ...
            case VarDeclStmt vds -> {
                ...
                // SEM048 fires only when the declared type is NOT nullable (no trailing '?')
