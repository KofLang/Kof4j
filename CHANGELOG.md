...
## [0.5.0-beta] - unreleased (branch `beta-0.5.0`)
  - **Fix — #631 (26/09): `Directory.list()` no JS devolvia o caminho COMPLETO de
    cada entrada em vez do NOME — `pasta + "/" + entrada` (o uso natural, o que a
    ...
    `IoDirListNamesE2ETest` (JVM+Script golden medidos; JS RED pre-fix / GREEN
    post). Catalog: known-bugs.md §518 (+PT).
  - **Fix — #630 (26/09): predicados do kof.io tipados BOOL (`File.exists`/
    `isFile`, `Directory.isDirectory`, `Path.isAbsolute`) devolviam o numero
    `1`/`0` no alvo JS — `String.valueOf` dava `"1"` e `List<Bool>.contains(true)`
    dava `false` em silencio (contexto `if` funcionava; a OBSERVACAO divergia da
    JVM/Native/Script)** — as 4 pontes restantes da familia §382 no `KofJsRunner`
  ...
  - **Feature — X2 fatia 1 (D-COMPLETE-FIRST item 2, 26/09): the Python engine `KofPy` in `kof.interop` — a Kof-written host, not a compiler face** —
    ...
    run `python3 -u -c <source+prelude> <spec>` through `process.spawn` (F10) with a
    2-line RPC (status + payload) decoded by the face's type (`json.decode<T>`).
    KOF-first (rule 10): the marshalling engine is `.kf` code (`interop-py-host.kf`),
    same pattern as the workflow host — zero new compiler surface beyond the
    target-gated injection in `CompilerInterop`. Session = the source (stateless
    replay, the honest contract until a named handle type exists — rule 6); live
...
- **feat: memory-safety Fase 3 fatia 3.1b — resource-lifetime warning
  ...
  - **`RISCV_RUNTIME_ASM_SHELL`/`RISCV_RUNTIME_ASM_SSH`/`RISCV_RUNTIME_ASM_PIPELINE` no longer compile-time constants (§257 guard)**
    (26/09): `bf5e3e03` (SHELL/SSH) and the row-2 slice B2 `NativeRiscvAsmPipeline`
    (PIPELINE, `RISCV_RUNTIME_ASM_PIPELINE`) left `public static final String` +
    text block, which javac inlines into consumers;
