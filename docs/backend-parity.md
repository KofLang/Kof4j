...
| `throw` dentro de `catch` (re-throw) no KofJS | ABERTO (bug 52) | JS `unexpected KofCatchStart` (corpo de catch termina em KofThrow, saída não cai no handler externo); JVM/Native/Script ok |
