...
| `String.length`/`charAt` no Native (UTF-8 vs UTF-16) | ABERTO (bug 43; cf. STR001) | Native conta bytes UTF-8 (`"café".length`=5, `charAt(3)`=195); JVM conta code units UTF-16 (4, 233). Decisão de design (regra 6) — alinhar convenção nos 3 targets |
