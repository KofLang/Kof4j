...
| `kof.math` (`clamp/abs/sign/min/max/isEven/...` Int + `sqrt/lerp/percentage/isInteger/isDecimal` Double) | ✅ | ✅ | ✅ | Escalares Int nos 4 alvos; **Double (S1b/S1b.1): JVM/Script/JS/x86 (SSE2 `sqrtsd`/`cvttsd2si`/`ucomisd`, bits crus via rax) + riscv64/aarch64 — MATH001 fechado 11/09** (fatia B36 `fsqrt.d`/`fmul.d`/`fdiv.d` + `fcvtzs`/`fcvt.d.l`; bits crus via a0..aN = modelo "rax cru" do x86; golden 20 vetores idêntico nos 5 targets sob qemu). `Double as Int` aarch64 corrigido junto (tradutor: `fcvt.w/l.{s,d}`→`fcvtzs`, era `scvtf` invertido — §104). `pow`/libm e `roundTo`-mode aguardam decisão da mantenedora. Bug 94: `NaN==NaN` só no interpretador (semântica congelada). |
| `kof.supervisor` (núcleo OTP issue #83: `import kof.supervisor` → `supervisor(name).child(id,fabrica,politica).restartLimit(max).escalate(cb).start()/.stop(deadlineMs)/.stats()`) | ✅ | 🔴 OTP001 (§109) | ✅ OTP002 (§112) | Host puro-Kof injetado como pacote virtual (resource `dev/kof/supervisor-host.kf`, mecanismo idêntico ao `android-host`: só age no `import kof.supervisor` explícito — zero falso-positivo). Observa falha do worker por `try{await}catch` por filho (laço `vigiar` em thread dedicada), reinicia com **fábrica nova** (DD-OTP-06), respeita restartLimit (para + R6 sem escalate), stop cooperativo `cancel`+deadline, stats(started/restarts/dropped/vivos). Gate DD-OTP-11: worker falha 2x → termina na 3ª → restarts=2/fabrica=3. NATIVE: §109 (throw em task longjmpa no `kof_exc_chain` global → crash/hang cross-thread) — bloqueio no compile-time com OTP001, NUNCA fallback. JS: §112 (event-loop single-thread não agenda task de task sem ceder — worker nunca roda) — OTP002. `KofSupervisorE2ETest` 6/6 (JVM+Script run, limite, gates OTP001/OTP002, sem-import não injeta). Impeditivo §110 corrigido junto (SEM024 falso em corpo de método re-analisado no mesmo escopo). |
...
| `String.length`/`charAt` no Native (UTF-8 vs UTF-16) | ✅ x86_64 + riscv64/aarch64 + JS (bug 43; cf. STR001) | O Native contava bytes UTF-8 (`"café".length`=5, `charAt(3)`=195); JVM conta code units UTF-16 (4, 233). x86_64 corrigido 10/09; JS 10/09; riscv64/aarch64 corrigidos 11/09 — `length`/`charAt` (fatia `NativeRiscvAsmRtB33`, trampolim `j` p/ `kof_su_length`/`kof_su_char_at`, decoder da B32) e `substring`/`indexOf`/`lastIndexOf` (fatia `NativeRiscvAsmRtB34`, `kof_su_walk` fiel ao x86 + `kof_su_substring`/`kof_su_index_of`/`kof_su_last_index_of`). Prova cross: 11 vetores length/charAt + 14 vetores substring/indexOf + bordas + panic astral, 4 targets idênticos (`NativeRiscv64/Aarch64E2ETest`, `assumeToolchain`). **Resíduo de contrato (x86, não cross):** `substring(0,0)`→`"ab"` vs JVM `""` — decisão da mantenedora (ver §43). **`Object.equals` (receiver `Object`) ainda dá link-fail — exige dispatch virtual no Native (gap §97).** |
| `String.compareTo`/`hashCode`/`equals` no Native | ✅ x86_64 + riscv64/aarch64 + JS (bug 97, 10–11/09) | Declarados no reference + aceitos pelo typer, mas os nativos não os emitiam (`undefined reference java_lang_String_compareTo`/`_hashCode`/`_equals`). x86_64 corrigido 10/09 (`RuntimeStringCompare`, code units UTF-16; `.equals` roteado p/ `kof_string_equals` c/ guard isString p/ não hijackar `record.equals`; golden `NativeE2ETest.nativeStringCompareToAndHashCodeUtf16` + `nativeStringEqualsVsRecordEquals`). JS corrigido 10/09 (`JsCallEmitter` cases + `kofStringCompareTo`/`kofHashCode` — golden `KofStringsTest.compareToAndHashCodeJvmJsNative`). riscv64/aarch64 corrigidos 11/09 (fatia `NativeRiscvAsmRtB32` — MESMO algoritmo de code-unit via `kof_su_next`; aarch herda do tradutor; `equals` cross roteado p/ `kof_string_equals` existente; guard no dispatcher já evita hijack; golden MESMOS 11 vetores em `NativeRiscv64E2ETest`/`NativeAarch64E2ETest` sob qemu). **Resíduo: `Object.equals` (receiver tipado Object) ainda dá link-fail — exige dispatch virtual no Native, gap aberto p/ lane Native.** |
| `println(double)` no Native x86_64 (6 casas, sem `.0`) | ABERTO (bug 44) | Native `0.333333`/`5`; JVM `0.3333333333333333`/`5.0`. Contradiz "x86_64 impecável" acima. Correção = lane Native (`RuntimePrintNum`/`kof_print_double`) |
