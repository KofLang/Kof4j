...
| `String.compareTo`/`hashCode`/`equals` no Native | ✅ x86_64 (bug 97) / ABERTO JS+cross | Declarados no reference + aceitos pelo typer, mas os nativos não os emitiam (`undefined reference java_lang_String_compareTo`/`_hashCode`/`_equals`). x86_64 corrigido 10/09 (`RuntimeStringCompare`, code units UTF-16; `.equals` roteado p/ `kof_string_equals` c/ guard isString p/ não hijackar `record.equals`; golden `NativeE2ETest.nativeStringCompareToAndHashCodeUtf16` + `nativeStringEqualsVsRecordEquals`). **Resíduo: `Object.equals` (receiver tipado Object) ainda dá link-fail — exige dispatch virtual no Native, gap aberto p/ lane Native.** JS: caem no default → `TypeError` (node ausente). riscv/aarch: símbolos só no `.s` x86. Correção das 2 faces = lanes JS/cross |
| `println(double)` no Native x86_64 | ✅ x86_64 (bug 44 10/09 + spelling inf/nan 11/09) / ABERTO riscv/aarch | 16 casas + `.0` (JDK `Double.toString`) + `Infinity`/`-Infinity`/`NaN` (não o `inf`/`nan` do glibc). **Caminho real do `println(double)` = box `kof_double_to_string` (`RuntimeStringConv`), NÂO `kof_print_double`** — o fix precisa tocar os 2. `print`/unbox = `RuntimePrintNum`. Prova `ConformanceMatrixTest.infinityprint`+`floatprint`; riscv/aarch = `FLT001` (cross) |
