...
| Native riscv64/aarch64 codegen | `NATIVE002` | **riscv64 core completo (02/09) + aarch64 core completo (03/09)**: `NativeRiscv64E2ETest 19/19` + `NativeAarch64E2ETest 19/19` — runtimes + codegen em **asm puro** (raw syscalls, **sem C**; `as`+`ld` estático com `--no-relax` (gp=0 sem C runtime); aarch64 via `translateRiscvToAarch64`) + qemu: println, var, if/else, aritmética, **classes (virtual dispatch/fields), arrays, List, switch, try/catch/throw, pattern matching (switch String s/instanceof/as), String methods, recursão, JSON (encode/decode record+listas), HTTP (get/post/put/patch/delete/options/status + headers — asm-generic socket=198/connect=203/write=64/read=63/close=57, mesma tabela nos 2), spawn/await (`clone(220)`+`futex` — qemu-riscv64 8.2.2 não tem clone3; heap compartilhado → `kof_alloc` bump atômico `amoadd.d`/`ldadd`)**. Paridade total x86 (DB/UI/net) pendente nos dois. **Ver `docs/native-multiarch.md`** |
