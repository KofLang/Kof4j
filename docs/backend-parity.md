...
| Strings (`+`, `==`, length, charAt, substring, contains, startsWith, endsWith, indexOf, trim, case, replace, split) | ✅ | ✅ | ✅ | `STR001` quase fechado: UTF-16 em x86_64 + JS + riscv/aarch `length`/`charAt` (bug 43, 10–11/09); **residual: riscv/aarch `substring`/`indexOf`/`lastIndexOf` ainda byte-based** (ver linha bug 43 acima) |
...
| `String.length`/`charAt` no Native (UTF-8 vs UTF-16) | ✅ x86_64 + riscv64/aarch64 (bug 43; cf. STR001) | O Native contava bytes UTF-8 (`"café".length`=5, `charAt(3)`=195); JVM conta code units UTF-16 (4, 233). x86_64 corrigido 10/09 (`RuntimeStringBase`/`RuntimeStringOps`); riscv64/aarch64 corrigidos 11/09 (fatia `NativeRiscvAsmRtB33` — trampolim `j` p/ `kof_su_length`/`kof_su_char_at`, reusa decoder da B32; golden 11 vetores + fuzz charAt 4 targets em `NativeRiscv64/Aarch64E2ETest.nativeStringLengthAndCharAtUtf16`). **Residual cross: `substring`/`indexOf`/`lastIndexOf` ainda byte-based no riscv/aarch (medido 11/09 — `substring(1,3)` corta é no meio → `b\uFFFD`; mesma face do §43 pendente)** |
