...
| `app.delete(...)` → COMP002 frame crash (GitHub #29) | ABERTO (bug 54) | `KofIo.instanceMethod` `case "delete"` sem `argCount==0` colide com rota web → `KofPop` extra sobre `kof_web_route` (void) → underflow ASM `Frame.merge`. `get/post/put/patch/options` ok. Frontend compartilhado (afeta alvo que baixar web). Correção = lane compiler |
| Handler web que termina em `return null` responde 404 (GitHub #28) | ABERTO (bug 53) | `ExpressionTyper` caso `LambdaExpr` só varre `ReturnStmt` top-level → `return` aninhado em `if` ignorado → `invoke()` void descarta valor → dispatch vê null→404. Frontend compartilhado. Correção = lane compiler/typer |
