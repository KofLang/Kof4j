...
| `String.compareTo`/`hashCode` no Native | ✅ x86_64 (bug 97) / ABERTO JS+cross | Declarados no reference + aceitos pelo typer, mas os nativos não os emitiam (`undefined reference java_lang_String_compareTo`/`_hashCode`). x86_64 corrigido 10/09 (`RuntimeStringCompare`, code units UTF-16, golden 11 vetores `NativeE2ETest.nativeStringCompareToAndHashCodeUtf16`). JS: caem no default → `TypeError` (node ausente, sem teste). riscv/aarch: símbolos só no `.s` x86. Correção das 2 faces = lanes JS/cross |
