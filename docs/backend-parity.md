...
| `kof.supervisor` (núcleo OTP issue #83: `import kof.supervisor` → `supervisor(name).child(id,fabrica,politica).restartLimit(max).escalate(cb).start()/.stop(deadlineMs)/.stats()`) | ✅ | 🔴 OTP001 (§123) | ✅ OTP002 (§126) | Host puro-Kof injetado como pacote virtual (resource `dev/kof/supervisor-host.kf`, mecanismo idêntico ao `android-host`: só age no `import kof.supervisor` explícito — zero falso-positivo). Observa falha do worker por `try{await}catch` por filho (laço `vigiar` em thread dedicada), reinicia com **fábrica nova** (DD-OTP-06), respeita restartLimit (para + R6 sem escalate), stop cooperativo `cancel`+deadline, stats(started/restarts/dropped/vivos). Gate DD-OTP-11: worker falha 2x → termina na 3ª → restarts=2/fabrica=3. NATIVE: §123 (throw em task longjmpa no `kof_exc_chain` global → crash/hang cross-thread) — bloqueio no compile-time com OTP001, NUNCA fallback. JS: §126 (event-loop single-thread não agenda task de task sem ceder — worker nunca roda) — OTP002. `KofSupervisorE2ETest` 6/6 (JVM+Script run, limite, gates OTP001/OTP002, sem-import não injeta). Impeditivo §124 corrigido junto (SEM024 falso em corpo de método re-analisado no mesmo escopo). |
