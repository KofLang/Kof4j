...
| `json.decode<List<Record>>` | ABERTO (bug 48) | JVM/JS ok; Native não compila (`JsonDispatch.decodeFunction` sem ramo lista-de-classe); interpretador exit 1 R6 (`Class.forName` p/ KofObj). `decode<Record>` no topo já ok nos 4 (fix 07/09 `KofInterpreterRuntime.decodeKofValue`) |
