...
> Deltas desde 0.1.0: Targets `native.risc` (riscv64) e `native.arm` (aarch64) separados de `native` x86_64 (**core completo 02-03/09** — `NativeRiscv64E2ETest 13/13` + `NativeAarch64E2ETest 13/13` em asm puro + qemu, paridade avançada pendente; ver `docs/development/native-multiarch.md`); Native free-list (`kof_free_head`) + `kof_gc_collect` (mark-sweep pendente; auto-GC desativado); MySQL wire protocol em progresso (`kof_db_mysql_scramble` + `user:pass@`); pattern matching `switch case String s` + record destructuring `Point(x,y)` em JVM/Native/JS; `String?` null safety básica; `KofScript` top-level `let` → `KofScriptGlobals`; `KofCcompiler` (`kof c`) native-only C subset; `List map/filter/reduce` + `Box<T>`; Windows SIGPIPE fix.
...
> Linhas "Deltas" registram o histórico por release (0.1.0→0.3.22-beta). Build atual: `mvn test` **1576** (1404 kof-compiler + 31 kof-script + 5 kof-c-compiler + 136 kof-cli), 0 falhas, 126 skip (guardas toolchain/node) — 12/09. A matriz reflete o estado corrente dos 3 targets. DoD em `docs/development/plan-platform-completion.md`.
...
| `kof.math` (`clamp/abs/sign/min/max/isEven/...` Int + `sqrt/lerp/percentage/isInteger/isDecimal` Double) | ✅ | ✅ | ✅ | Escalares Int nos 4 alvos; **Double (S1b/S1b.1): JVM/Script/JS/x86 (SSE2 `sqrtsd`/`cvttsd2si`/`ucomisd`, bits crus via rax) + riscv64/aarch64 — MATH001 fechado 11/09** (fatia B36 `fsqrt.d`/`fmul.d`/`fdiv.d` + `fcvtzs`/`fcvt.d.l`; bits crus via a0..aN = modelo "rax cru" do x86; golden 20 vetores idêntico nos 5 targets sob qemu). `Double as Int` aarch64 corrigido junto (tradutor: `fcvt.w/l.{s,d}`→`fcvtzs`, era `scvtf` invertido — §120). `pow`/libm e `roundTo`-mode aguardam decisão da mantenedora. Bug 94: `NaN==NaN` só no interpretador (semântica congelada). |
| `kof.supervisor` (núcleo OTP issue #83: `import kof.supervisor` → `supervisor(name).child(id,fabrica,politica).restartLimit(max).escalate(cb).start()/.stop(deadlineMs)/.stats()`) | ✅ | 🔴 OTP001 (§129) | ✅ OTP002 (§132) | Host puro-Kof injetado como pacote virtual (resource `dev/kof/supervisor-host.kf`, mecanismo idêntico ao `android-host`: só age no `import kof.supervisor` explícito — zero falso-positivo). Observa falha do worker por `try{await}catch` por filho (laço `vigiar` em thread dedicada), reinicia com **fábrica nova** (DD-OTP-06), respeita restartLimit (para + R6 sem escalate), stop cooperativo `cancel`+deadline, stats(started/restarts/dropped/vivos). Gate DD-OTP-11: worker falha 2x → termina na 3ª → restarts=2/fabrica=3. NATIVE: §129 (throw em task longjmpa no `kof_exc_chain` global → crash/hang cross-thread) — bloqueio no compile-time com OTP001, NUNCA fallback. JS: §132 (event-loop single-thread não agenda task de task sem ceder — worker nunca roda) — OTP002. `KofSupervisorE2ETest` 6/6 (JVM+Script run, limite, gates OTP001/OTP002, sem-import não injeta). Impeditivo §130 corrigido junto (SEM024 falso em corpo de método re-analisado no mesmo escopo). |
| `kof.observability` (health/metrics/requestId) | ✅ | ✅ | ✅ | `KofObservabilityTest` |
| `kof.http` client | ✅ | Native HTTP/1.1 asm (socket/recv/send parse; https→throw; DNS não-IP→127.0.0.1; retry knob no-op) | ✅ GraalJS síncrono (`Java HttpClient` interop); Node/browser = fetch real **assíncrono** (`spawn http.* + await` → Promise; §133 — a face síncrona em JS puro fica HTTP003) | `KofHttpE2ETest` 8/8 (JVM+JS+Native real resp; `jsNodeSpawnAwaitHttpResolvesBody` no node); retry/circuit paridade 30/08 |
...
| Campo estático no Native | ABERTO (bug 41) | `nat/NativeBackend:629-630` stub vazio p/ `KofGetStatic`/`KofPutStatic` (R6 silencioso) — imprime memória não-inicializada; JVM/JS/interp dão `1\n2\n2`. Correção = lane Native (emitir com offset de dados real) |
| `record.hashCode()` no JS/Native | ABERTO (bug 42) | JS: `TypeError: hashCode is not a function`; Native: `ld: undefined reference to P_hashCode`. `==`/`toString` têm paridade — só `hashCode` diverge. Correção = lane JS+Native |
...
| `println(double)` no Native x86_64 | ✅ x86_64 (bug 44 10/09 + spelling inf/nan 11/09) / ABERTO riscv/aarch | 16 casas + `.0` (JDK `Double.toString`) + `Infinity`/`-Infinity`/`NaN` (não o `inf`/`nan` do glibc — spelling no `RuntimePrintNum`, presente no merge). **Caminho real do `println(double)` = box `kof_double_to_string` (`RuntimeStringConv`), NÃO `kof_print_double`** — o fix toca os 2. Prova `ConformanceMatrixTest.infinityprint`+`floatprint`; riscv/aarch = `FLT001` (cross) |
| `finally` c/ `return` no try: JS perde retorno; JVM/Native/interp descartam finally | ABERTO (bug 45; regra 6 p/ o resto) | JVM/Native/interp → `1` (finally descartado, consistente = congelado); JS → `finally` roda mas retorno `undefined` (bug JS). finally **roda** p/ throw e no-return |
| `json.decode<List<Record>>` | ABERTO (bug 48 — só Native) | Native não compila (`JsonDispatch.decodeFunction` sem ramo lista-de-classe); JVM/JS/interpretador ok (metade interpretador corrigida 07/09 em `KofInterpreterRuntime`). `decode<Record>` no topo ok nos 4 (fix 07/09 `KofInterpreterRuntime.decodeKofValue`) |
...
| channel send/recv DENTRO de `spawn` | ABERTO (bug 50) | Native SIGSEGV 139 (futex do canal fora da thread principal; canal-sem-spawn e spawn-sem-canal ok); JVM/Script/JS ok |
...
| REGRESSÃO Native riscv64/aarch64: `println` → `undefined reference kof_static_java_lang_System_out` (59 testes) | ABERTO (bug 59, lane Native) | desde `62423bf` (fix bug 41): `.s` riscv/aarch referencia o símbolo mas não o define no `.data` (x86 define). Bissect: verde `4a073ff`, vermelho `62423bf`. `nat/` EM CURSO no REFACTOR-500 — não corrigido por esta lane (regra 2/3) |
