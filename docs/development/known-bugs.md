...
- **⚠️ Residual (lane Native riscv64/aarch64 — `substring`/`indexOf`/`lastIndexOf` ainda byte-based):** medido 11/09 — `"ab\u00e9cd".substring(1,3)` riscv=`b\uFFFD|` (corta o é no meio) vs JVM=`b\u00e9|`; `"\u00e9x\u00e9".lastIndexOf("\u00e9")` riscv=3 (byte) vs JVM=2 (unit); `"\u00e9abc".indexOf("a")` coincidência ASCII (ambos 2). Mesmo furo que o x86 teve (aqui ainda aberto): as 3 faces no RISC-V (`Rt1` substring, `Rt1`/`RtStrn0` index_of/last_index_of) precisam do walk de code-unit (reusar `kof_su_next`/padrão `.Ls43*`). **Nota:** o `ConformanceMatrixTest` (`unicode*`/`unicode-indexof`) NÃO pega isso — a matriz só roda JVM/x86/Script/JS, nunca riscv/aarch; a prova cross vive em `NativeRiscv64E2ETest`/`NativeAarch64E2ETest` (com `assumeToolchain`). Próxima unidade da lane cross: substring/indexOf/lastIndexOf UTF-16 riscv.
