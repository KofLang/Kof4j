...
| `entity-history-plan.md` | **Entity History — temporal audit for `kof.orm`**: opt-in per entity (`audited entity`), append-only revisions per entity (`INSERT/UPDATE/DELETE`) with actor/time/correlation, entity-static query surface (`User.history/at/atTime/changes`), snapshot-per-revision, writes inside `transaction { … }`, capability model + `HIST0xx` codes | **plan only, zero code** — maintainer-requested 26/09; SPEC ONLY (contract, no implementation); grounded in the measured ORM/DB stack (`KofOrm.java:137`, `KofDb.java:80`, `JvmOrmRuntime.java:89`, `CompilerPipeline.java:228-229`, `ExpressionParser.java:149`, `CompilerInterop.java:17-19`); no history/revision/temporal mechanism exists; rule 6 — promotion needs a `D-ENTITY-HISTORY` decision (15 open questions in §27) |
