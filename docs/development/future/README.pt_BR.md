...
| `entity-history-plan.md` | **Histórico de Entidades — auditoria temporal para `kof.orm`**: opt-in por entidade (`audited entity`), revisões append-only por entidade (`INSERT/UPDATE/DELETE`) com ator/tempo/correlação, superfície de consulta entity-static (`User.history/at/atTime/changes`), snapshot-por-revisão, escritas dentro de `transaction { … }`, modelo de capacidade + códigos `HIST0xx` | **só plano, zero código** — registrado a pedido da mantenedora 26/09; SPEC APENAS (contrato, sem implementação); ancorado na stack ORM/DB medida (`KofOrm.java:137`, `KofDb.java:80`, `JvmOrmRuntime.java:89`, `CompilerPipeline.java:228-229`, `ExpressionParser.java:149`, `CompilerInterop.java:17-19`); não existe mecanismo de histórico/revisão/temporal; regra 6 — promoção exige decisão `D-ENTITY-HISTORY` (15 open questions no §27) |
