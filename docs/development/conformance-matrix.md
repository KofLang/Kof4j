...
| `println(coleção)` formato do contêiner | `[1, 2]` / `[a, b]` / `[1.5, 2.25]` / `{k=1}` / `[1]` / `[Point[..], Point[..]]` / `[[1], [2]]` | DONE | PARTIAL (bug 106: ponteiro cru — vtable `-1` silencioso em kof.List/Map/Set) | DONE | DONE (bug 106-JS ✅ 11/09 — `kofFormat` espelha ArrayList/HashMap/HashSet.toString) | `collprint` |
