...
|---|---|---|---|---|---|
| **NATIVE002-stdlib (residual R6)** — auditoria de falha silenciosa cross | `EM CURSO` | melissa (agente) | `beta-0.3.0` | `NativeBackend.java` (riscv asm), `KofScheduler.java`, `KofTime.java`, `KofMq.java` | 05/09: **fcvt riscv64** (`1a2f044`); **ToolchainMissing** — falha de as/ld vira erro de compilação (R6 na fonte); **FLT001** — `println(double)` no cross → diagnóstico (`2a7e89f`); **time.now()** real clock_gettime=113 (`a67a8de`); **cache** real + `println(null)` + `sle/sge` inválidos (`0e6d0f9`); **MQ001 cross FEITO** (`05d0d1d`); **tail-call em 8 funções** riscv (`fc34bc7`: call+ret sem salvar ra = loop infinito); **gates SCHED001+TIME001** (`0c4e4c5`); **"42".toInt()** SIGSEGV (deref do valor do char — `696c6c9`); **Map/Set cross FEITO** (`93fec3f`+`858718e`: RISCV_MAPSET_ASM linear-scan, paridade exata) + **kof_panic** imprime C-string (mensagem bounds); **higher-order cross FEITO** (`7b81871`: map/filter/reduce, closure ABI do mq). Sweep KSw2: **0 divergências** nos 3 targets. Merges `df191e0`/`5aa6870` (Fases 2 do refactor — gates FLT001/SCHED001/TIME001 sobreviveram, 159/159 verde). Pendente: paridade `metrics()` (# TYPE, cosmético) — ver PRÓXIMO PASSO. |
| **REFACTOR-500** — dividir as 20 classes >500 linhas (regra ≤500) | `EM CURSO` | divisão multi-agente | `beta-0.3.0` | plano `docs/refactoring/PLAN-SOLID-500.md` | **Divisão**: agente-idiomatic faz **Fases 1–3 + 9** (`NativeRuntime`, `CompilerDriver`, `NativeBackend`, varredura); **fixes-for-kofagent faz Fases 4–8** (`JsBackend`, `JvmRuntime`, `SemanticAnalyzer`, `Parser`, classes 500–1400) (`JvmRuntime`, `SemanticAnalyzer`, `Parser`, classes 500–1400); agente-idiomatic fecha com Fase 9 (varredura final). **Contrato DRY**: `TypeMapper`/`NativeNameMangler`/`TypeMetrics` são criados por agente-idiomatic e consumidos (nunca recriados) por fixes-for-kofagent. Cada fase = commit isolado + suíte completa verde como gate. ⚠️ Nunca dois agentes no mesmo arquivo gigante. **Progresso agente-idiomatic 05/09: FASE 1 COMPLETA** — NativeRuntime 17726→142 linhas (só orquestrador); ~60 classes Runtime* ≤500. Suíte compilador 922 testes, 0 falhas. **fixes-for-kofagent**: fix println(char) (`94aca7a`), iniciando JvmRuntime. **PRÓXIMO PASSO (agente-idiomatic)**: Fase 2 CompilerDriver — F2.1-F2.4 FEITOS (TypeMetrics `f80211d`, StringMethodRegistry `497fe3a`, TypeEmitter `02ed40f`, CompilerTypes `6114cd8`; CompilerDriver 8870→8642). **PRÓXIMO**: F2.5 extrair BoxClassFactory (createBoxClass — precisa levar boxCounter/boxClassNames/boxValueTypes/syntheticClasses num record BoxState) e CompilerImports/CompilerDesugar (expandKofImports/desugarTests/desugarApplication). Depois F2.6 LambdaLowerer+LambdaState, F2.7 SuperBridgeBuilder/QueryDslLowerer/UiEmitter/ArgumentsEmitter, F2.8 StatementLowerer/ExpressionLowerer com LoweringContext (o mais arriscado). Suíte completa (927 testes) verde como gate.
