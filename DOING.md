...
**PRÓXIMO PASSO (09/09, lane bug-fix — 65 bugs a 100%):** **Estado dos bugs após esta sessão (commit `449ac4c9`):** corrigidos no código — **48** (json.decode<List<Record>> Native → gap honesto JSN004 em ExpressionJsonCallLowerer + panic em RuntimeJsonDecode), **59** (regressão riscv/aarch `undefined reference kof_static_java_lang_System_out` — NativeArchEmitter emitRiscv/emitAarch64 agora chamam collectStaticFields+emitStaticData), **62(a)** (val imutável → SEM037: StatementParser carrega type="val", LocalVariableSymbol.isVal, analyzeAssignmentStatement). Marcados como já-corrigidos no doc (com teste): **7, 9, 18, 21, 22, 23, 30, 43, 44, 63, 64**. Bug 61 = gap honesto FFI001 já implementado (isExternBound p/ NATIVE). Bug 46 = teste `nativeSpawnExprAwaitLambdaReturn` adicionado (estado a confirmar com toolchain). **Restam ABERTOS (exigem ambiente ou regra 6):** 39 (Map.get nullable — bump), 45 (finally return — DD-01 bump), 50 (channel+spawn Native — qemu/valgrind), 62(b)/(c) (escrita em campo de record — checagem de FieldAccess), 65 (Audio/Video DOM browser — lane UI). **PRÓXIMA TAREFA:** validar commits com `mvn test` (ferramenta ausente no ambiente atual); depois atacar 50 (futex do canal na thread do spawn) ou 62(b)/(c).
