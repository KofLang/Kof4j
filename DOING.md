...
|---|---|---|---|---|---|
| **APP-MODEL** — Kof Application Model (RFC + plano: monólito full-stack ↔ distribuído, única abstração, `kof.toml`, packaging, System/Workspace) | `EM CURSO` | agente-app-model | `beta-0.3.0` | `docs/future/APPLICATION_MODEL.md` (novo, em construção), `DOING.md` | 05/09: auditoria do estado atual FEITA (CLI `CmdServe`/`CmdBuild`/`Deps`, stdlib web/http, targets, gaps WEB00x/HTTP003, roadmap §§8–11). Escrito §1–2 (motivation + auditoria). **PRÓXIMO:** §3–12 (RFC: principles, application, topology, build/runtime/deploy, CLI, targets, security, tests, open questions) + §13 plano de implementação por incrementos (I1 manifesto, I2 full-stack `kof serve`, I3 packaging, I4 System) |
