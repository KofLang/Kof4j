...
| **REFACTOR-500** — dividir as 20 classes >500 linhas (regra ≤500) | `EM CURSO` | divisão multi-agente | `beta-0.3.0` | plano `docs/refactoring/PLAN-SOLID-500.md` | **Divisão (confirmada pelo humano 05/09)**: agente-idiomatic faz **Fases 1–3 + 9** (`NativeRuntime`, `CompilerDriver`, `NativeBackend`, varredura); **fixes-for-kofagent faz Fases 4–8** (`JsBackend`, `JvmRuntime`, `SemanticAnalyzer`, `Parser`, classes 500–1400). **Contrato DRY**: `TypeMapper`/`NativeNameMangler`/`TypeMetrics` são criados por agente-idiomatic e consumidos (nunca recriados) por fixes-for-kofagent. Cada fase = commit isolado + suíte completa verde como gate. ⚠️ Nunca dois agentes no mesmo arquivo gigante. **Progresso agente-idiomatic**: **FASE 1 COMPLETA 05/09** — NativeRuntime 17726→142 linhas (só orquestrador); ~60 classes `Runtime*` ≤500 (Print/StringConv/StringParse/StringSearch/StringOps/StringEdit/StringBase/List/Array/Json{Builder,Encode,Decode,ArrayDecode1-2,Utils}/Memory/Gc/Concurrency/Channel/Scheduler/Mq/Log1-2/Config1-2/Io1-3/Cache/Time/Ui/Net/Vk/Misc/PrintNum/Security1-11+Data/Validation/Observability1-3/Enum/Map/Set). Suíte compilador 922 testes, 0 falhas. **Pendente F2 (CompilerDriver)**: F1.15 dividiu métodos gigantes; próximo passo iniciar F2. **fixes-for-kofagent progresso**: linha reivindicada 05/09; fix da regressão `println(char)` do merge M36 (`94aca7a`, gap 27 aberto em known-bugs); iniciando Fase 5 (JvmRuntime). |
