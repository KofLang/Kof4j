...
| **NATIVE002-stdlib** — JSON/spawn/http/db no runtime riscv64 (aarch64 herda via tradutor) | `EM CURSO` | agente-planning | `beta-0.3.0` | `NativeBackend.java` (`RISCV_RUNTIME_ASM`), `NativeRuntime.java` (x86_64) | 04/09: **JSON completo** ✅ (quote + encode/decode de record/lista/primitiva — riscv64+aarch64 16/16 qemu). **Bug x86_64 corrigido**: `movw` de escape gravava bytes errados (0x1514→`14 15`); valores LE corretos (paridade com JVM). Falta: spawn/await (`clone`+`futex` — runtime é estático, sem pthread), http (sockets), db. |
