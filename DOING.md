...
|---|---|---|---|---|---|
| **F9-CONFORMANCE** — matriz Feature × target (plano plataforma Fase 9; roadmap-audit P4 "Conformance Suite NOT STARTED") | `EM CURSO` | lane KOFSCRIPT (fixes-for-kofagent) | `beta-0.3.0` | `docs/development/conformance-matrix.md`, `ConformanceMatrixTest.java` (kof-compiler/src/test), `KofInterpreterRuntime.java`, `DOING.md` | 07/09: **LOTE 1 + LOTE 2 FEITOS**. `ConformanceMatrixTest` (10 testes) trava a MESMA saída nos 4 targets (JVM/Native/Script/KofJS) — 40 casos determinísticos (linguagem core + erros/null/JSON). Células PARTIAL = bug registrado (41-45, 48, 49) + ref na matriz. **FIX da lane (este commit):** `json.decode<Record>` no interpretador dava exit 1 + stderr só `Point` (R6) — o método gerado faz `Class.forName`, mas no interpretador a classe Kof é `KofObj`; corrigido em `KofInterpreterRuntime.decodeKofValue` (espelha `encodeKof`); prova `KofScriptTest.jsonDecodeRecordRunsOnInterpreter` + caso `jsondec-record` DONE×4. **ACHADOS novos:** bug 48 (`json.decode<List<Record>>` — interpretador exit 1 R6 + Native não compila; JVM/JS ok) e bug 49 (KofJS não compila `try` aninhado — COMP002 `try expected KofTryEnd`; JVM/Native/Script ok). Matriz em `docs/development/conformance-matrix.md` (lotes 1+2 preenchidos). Suíte 1078/0/3-skip. **PRÓXIMO PASSO (F9):** lote 3 = spawn/await determinístico + channel (travar nos 4, excluir onde há bug 29/46); depois estender p/ riscv64/aarch64 via qemu (harness `NativeRiscv64E2ETest`); matriz vira gate de CI (comparar células × testes reais). |
