...
| **TIER 6-12 super sessão** — mapear/implementar o que o REFACTOR-500 não bloquear | `EM CURSO` | agente-planning | `planning-future` | `docs/future/blockers/BLOCKERS.md`, `docs/future/NAMESPACE-RUNBOOK.md` | 06/09 pós-merge: **TIER 6 JVM-first FEITO** — `kof.workflow` (`job(name){}/run/pipeline`, gap WF001) + `kof.shell` (`run(cmd)` via `sh -c`, gap SHL001), ambos na arquitetura modular (typers/lowerers/runtime JVM). Restante dos tiers: lados Native/JS + namespaces de instância exigem F2/F3 (`ExpressionMethodCallLowerer` + `NativeBackend`), dono agente-idiomatic. Retomada: ordem em `BLOCKERS.md`. |
