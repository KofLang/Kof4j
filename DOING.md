...
| **TIER 6-12 super sessão** — mapear/implementar o que o REFACTOR-500 não bloquear | `EM CURSO` | agente-planning | `planning-future` | `docs/future/blockers/BLOCKERS.md`, `docs/future/NAMESPACE-RUNBOOK.md` | 06/09 pós-merge: **TIER 6 `kof.workflow` JVM-first FEITO** (`job(name){}/run/pipeline` + gap WF001 Native/JS) na arquitetura modular (typers/lowerers/runtime JVM). Restante dos tiers exige F2/F3 (dispatch de instância em `ExpressionMethodCallLowerer` + `NativeBackend`), com dono agente-idiomatic. Retomada: ordem em `BLOCKERS.md`. |
