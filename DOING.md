...
|---|---|---|---|---|---|
| **F9-CONFORMANCE** — matriz Feature × target (plano plataforma Fase 9; roadmap-audit P4 "Conformance Suite NOT STARTED") | `EM CURSO` | lane KOFSCRIPT (fixes-for-kofagent) | `beta-0.3.0` | `docs/development/conformance-matrix.md` (novo), `ConformanceMatrixTest.java` (novo, kof-compiler/src/test), `DOING.md` | 07/09 (reivindicado): o roadmap-audit citava `BackendParityTest` (JVM×JS) como proxy; a Fase 9 pede matriz **Feature × {JVM, Native, Script(interp), KofJS}** com DONE/PARTIAL/UNSUPPORTED + teste que trava a matriz. Escopo: casos DETERMINÍSTICOS (sem tempo/concorrência), 4 targets (interp/JVM compilado/JS/Native x86_64 — riscv/aarch via qemu fica como follow-up); divergências = PARTIAL com ref de bug em known-bugs.md (41-47). Semântica congelada intocada: o teste trava o COMPORTAMENTO, não muda nada. **PRÓXIMO PASSO**: lote 1 = linguagem core (aritmética, strings, coleções, records, classes, controle, switch-expr) → lote 2 = erros/null/JSON; cada lote = commit + suíte verde. |
