...
|---|---|---|---|---|---|
| **NATIVE002-stdlib (residual R6)** — auditoria de falha silenciosa cross | `EM CURSO` | melissa (agente) | `beta-0.3.0` | `NativeBackend.java` (riscv asm), `KofScheduler.java`, `KofTime.java`, `KofMq.java` | 05/09: **fcvt riscv64** corrigido (`1a2f044`); **ToolchainMissing** — falha de as/ld vira erro de compilação (R6 na fonte, `1a2f044`); **FLT001** — `println(double)` no cross → diagnóstico (`2a7e89f`); **time.now()** real clock_gettime=113 (`a67a8de`); **cache** real + `println(null)` + `sle/sge` inválidos (`0e6d0f9`); **MQ001 cross FEITO** — port completo do kof.mq riscv64/aarch64 (`05d0d1d`: scan loops c/ ponteiro-fim, s-regs salvos, `kof_list_remove`, queue por handle, unsubscribe por identidade; gate removido); **tail-call em 8 funções** riscv (`fc34bc7`: `call`+`ret` sem salvar `ra` = loop infinito — health/ids/time_interval hangavam); **gates SCHED001 + TIME001** no cross (`0c4e4c5`: scheduler UNDEF no link + time.interval stub no-op silencioso). Merge `5764b2a` (FASE 1 do refactor — 146/146 verde pós-merge). Pendente: paridade `metrics()` (# TYPE) — ver PRÓXIMO PASSO. |
| **REFACTOR-500** — dividir as 20 classes >500 linhas (regra ≤500) | `EM CURSO` | divisão multi-agente | `beta-0.3.0` | plano `docs/refactoring/PLAN-SOLID-500.md` | **Divisão**: agente-idiomatic faz **Fases 1–3 + 9** (`NativeRuntime`, `CompilerDriver`, `NativeBackend`, varredura); **fixes-for-kofagent faz Fases 4–8** (`JsBackend`, `JvmRuntime`, `SemanticAnalyzer`, `Parser`, classes 500–1400) (`JvmRuntime`, `SemanticAnalyzer`, `Parser`, classes 500–1400); agente-idiomatic fecha com Fase 9 (varredura final). **Contrato DRY**: `TypeMapper`/`NativeNameMangler`/`TypeMetrics` são criados por agente-idiomatic e consumidos (nunca recriados) por fixes-for-kofagent. Cada fase = commit isolado + suíte completa verde como gate. ⚠️ Nunca dois agentes no mesmo arquivo gigante. **Progresso agente-idiomatic 05/09: FASE 1 COMPLETA** — NativeRuntime 17726→142 linhas (só orquestrador); ~60 classes Runtime* ≤500. Suíte compilador 922 testes, 0 falhas. **fixes-for-kofagent**: fix println(char) (`94aca7a`), iniciando JvmRuntime. **PRÓXIMO PASSO (agente-idiomatic)**: Fase 2 — CompilerDriver 8870→8717 (F2.1 TypeMetrics, F2.2 StringMethodRegistry); próximo TypeEmitter/CompilerTypes/BoxClassFactory. **Progresso agente-idiomatic (Fase 1, NativeRuntime 17726→~15900 linhas, 05/09)**: F1.1 `RuntimePrint` (`d4d7e69`), F1.2 `RuntimeStringConv` (`be4dc44`), F1.3 `RuntimeStringParse` (`ebced13`), F1.4 `RuntimeStringSearch` (`805793a`), F1.5 `RuntimeStringOps`+`RuntimeStringEdit` (`2e55fce`), F1.6 `RuntimeStringBase` (`9f4ef02`), F1.7 `RuntimeList` (`d3bc0f7`), F1.8 `RuntimeArray` (`23f4651`), F1.9 `RuntimeJson{Builder,Encode,Decode}` (`561fc10` — corrigiu label `.Lkof_json_true`), F1.10 `RuntimeMemory`+`RuntimeGc`, F1.11 `RuntimeConcurrency`+`RuntimeChannel`+`RuntimeScheduler`+`RuntimeMq`, F1.12 `RuntimePrintNum`+`RuntimeNet`+`RuntimeVk`+`RuntimeMisc`, F1.13 security/validation/observability/map/set/enum, F1.14 dedup, F1.15 divide métodos gigantes restantes. **FASE 1 COMPLETA 05/09**: NativeRuntime 17726→142 linhas (só orquestrador); ~60 classes Runtime* ≤500. Suíte compilador 922 testes, 0 falhas. **Pendente F1**: Gc/Core/Alloc, Concurrency/Channel/Scheduler/Mq, Db/Log/Config/Time/Cache/Io, Net/Security/Ui/Vk, PrintNum, orquestrador final. **Nota**: corrigi também regressão de paridade do merge `15f87b4` (fixes-for-kofagent) — `println(char)` imprimia o caractere; JVM imprime o código (72). |
 (docs(doing): REFACTOR-500 — F2.1/F2.2 CompilerDriver (TypeMetrics, StringMethodRegistry))
