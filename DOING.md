...
| **NATIVE002-stdlib** — JSON/spawn/http/db no runtime riscv64 (aarch64 herda via tradutor) | `EM CURSO` | agente-planning | `beta-0.3.0` | `NativeBackend.java` (`RISCV_RUNTIME_ASM`), `NativeRuntime.java` (x86_64) | 04/09: `kof_json_quote` portado (record String/Int/Long/Bool ✅ riscv64+aarch64 15/15 qemu). **Bug x86_64 corrigido**: `movw` de escape gravava bytes errados (0x1514→`14 15`); valores LE corretos agora (paridade com JVM). Falta: spawn/await (`kof_spawn`/`kof_await`/`kof_spawn_result`), http (sockets), db. Testes: `riscv64JsonEncode`/`aarch64JsonEncode` |
