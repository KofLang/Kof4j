...
**EM CURSO (09/09, lane spec-gaps — 2ª rodada de decisões do maintainer):** SG-002 FEITO (tokens mortos removidos do Lexer: TILDE/COLON_COLON/ELLIPSIS/DOUBLE_ARROW/PIPE_LINE/UNDERSCORE + keywords sealed/permits — enum TokenType limpo; `=>`/`::`/`|>` agora lexam como chars simples e falham no parse, `~`/`sealed` dão LEX005; prova `deadTokensGiveCleanLexerError` 5 casos). **SG-011B FEITO (SEM047):** sobrecarga top-level homônima → erro de compilação "is already defined... cannot be overloaded" (antes a última sobrescrevia silenciosamente — repro compilava e `f(1)` respondia o resultado da 2ª f); checagem no início de `SemanticAnalyzer.analyze` varrendo declarações; prova `duplicateTopLevelFunctionFails`. **DECISÃO REFORÇADA (SG-005/008 — null safety):** "o próprio nome já diz" — NENHUM literal `null` é atribuível (nem a `T?`): `Int? x = null` → erro; `x = null` → erro; E `Int? == null` não dá NPE (comparação de referência, sem unbox). **FILA:** SG-008 (bug NPE + ban de null literal, SEM048), SG-020 (validar provas + docs), SG-005 (deref T? sem narrowing → erro), SG-009 (subtipagem nominal em isAssignable). Suíte: 1238 run / 0 falhas MINHAS / 13 pré-existentes (12 + KofRandomTest de outra lane: export JS kofRandomInt pendente).
