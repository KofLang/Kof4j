...
| **NATIVE002-stdlib** — JSON/http/spawn/db no runtime riscv64 (aarch64 herda via tradutor) | `EM CURSO` | agente-planning | `beta-0.3.0` | `NativeBackend.java` (`RISCV_RUNTIME_ASM`, `emitRiscvHttp`), `NativeRuntime.java` (x86_64) | 04/09: **JSON completo** ✅ (riscv64+aarch64 16/16 qemu). **http completo** ✅ (`get/post/put/patch/delete/options/status` + headers — riscv64+aarch64 17/17 qemu; syscalls asm-generic socket=198/connect=203/write=64/read=63/close=57, mesma tabela nos 2). **Root cause de "http não funciona"**: bug de **gp-relaxation** no codegen riscv64 — `la` virava `addi rd,gp,off` com gp=0 (binário estático, sem C runtime) → fault; JSON passava por sorte de layout. Fix: `-mno-relax` no as + `--no-relax` no ld. **Fix tradutor aarch64**: `movz` (não `mov`) quando `lsl #16` (ex.: 262144). Falta: spawn/await (`clone`+`futex` — runtime é estático, sem pthread), db (sqlite). |
