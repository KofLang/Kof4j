...
| **TIER 6-12 super sessão** — mapear/implementar o que o REFACTOR-500 não bloquear; blockers explícitos p/ resto | `EM CURSO` | agente-planning | `planning-future` | `docs/future/blockers/BLOCKERS.md`, `docs/status.md`, `docs/roadmap.md` | 05/09: coordenação atualizada (`status.md`/`roadmap.md`). Blocker estrutural: todos os tiers exigem novos namespaces stdlib → dispatch no `CompilerDriver`/`SemanticAnalyzer`/`JvmRuntime` (REFACTOR-500 EM CURSO por outros agentes). Sem tocar nos gigantes, só há design/plan e prototypes de runtime em classes novas. |
