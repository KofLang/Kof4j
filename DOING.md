...
| **REFACTOR-500** — dividir as 20 classes >500 linhas (regra ≤500) | `EM CURSO` | divisão multi-agente | `beta-0.3.0` | plano `docs/refactoring/PLAN-SOLID-500.md` | **Divisão**: agente-idiomatic faz **Fases 1–3 + 9** (`NativeRuntime`, `CompilerDriver`, `NativeBackend`, varredura); **fixes-for-kofagent faz Fases 4–8** (`JsBackend`, `JvmRuntime`, `SemanticAnalyzer`, `Parser`, classes 500–1400) (`JvmRuntime`, `SemanticAnalyzer`, `Parser`, classes 500–1400); agente-idiomatic fecha com Fase 9 (varredura final). **Contrato DRY**: `TypeMapper`/`NativeNameMangler`/`TypeMetrics` são criados por agente-idiomatic e consumidos (nunca recriados) por fixes-for-kofagent. Cada fase = commit isolado + suíte completa verde como gate. ⚠️ Nunca dois agentes no mesmo arquivo gigante. **Progresso agente-idiomatic 05/09: FASE 1 COMPLETA** — NativeRuntime 17726→142 linhas (só orquestrador); ~60 classes Runtime* ≤500. Suíte compilador 922 testes, 0 falhas. **fixes-for-kofagent**: fix println(char) (`94aca7a`), gap 27 (valueOf char paridade). **FASE 5 FEITA** (`01af2d5`): JvmRuntime 2526→132 + 7 classes ≤500, source gerado byte-idêntico (prova por dump reflection). **FASE 8 FEITA** (merges `b6fb9d7`/`20b4726`+script): 13 classes 503–1401 → todas ≤500 (JvmString/Vk/Web/Media, NativeHttp/Web, Optimizer, Bench, Main, KofScript, KofJsRunner, JdwpClient), geradores byte-idênticos, suíte 943/0. Guard qemu adicionado (`4408eb6`). **PRÓXIMO PASSO (fixes-for-kofagent)**: FASE 7 Parser (1975→~4: StatementParser/ExpressionParser/TypeParser) → FASE 6 SemanticAnalyzer (2293→~5) → FASE 4 JsBackend (6064→~12). Suíte completa verde como gate em cada. **PRÓXIMO PASSO (agente-idiomatic)**: F2.24-F2.32 FEITOS (StaticCallLowerer, UiStaticLowerer, PrintLowerer, DbCallLowerer, ProcessCallLowerer, SchedulerCallLowerer, LogCallLowerer, 5 namespaces http/time/mq/config/cache, UiMediaCallLowerer). ExpressionMethodCallLowerer 1969→1033. ⚠️ LIÇÃO 05/09: o bloco de instância (`else if (mc.receiver() != null)`, ~490 linhas) tem if-super + `} else {` aninhado com indentação inconsistente — a extração por reindentação quebra o balanceamento. **PRÓXIMO**: extrair o dispatch de instância (o else interno, 627-1032) para ExpressionInstanceDispatchLowerer MANTENDO a indentação original e corrigindo os `}` ponto a ponto; ou dividir o if-super e o else em 2 métodos. Depois CompilerDriver final ≤500 e F3 (NativeBackend). Suíte completa verde como gate.
| **LANG-SPEC** — Language Reference Manual: separar linguagem Kof do compilador (auditoria + spec formal extraída do código, sem mudar comportamento) | `FEITO` | agente-spec | `beta-0.3.0` | `docs/language-reference/*` (novo), `docs/compiler-architecture.md` (novo), `docs/specification-gaps.md` (novo), `README.md`, `docs/architecture.md` | 06/09 **FEITO** (commits `f9c88bc`..`db3f472` + este): auditoria completa (lexer 477L, parser 1975L, AST 39 nós, SemanticAnalyzer, Type 8 records, IR 30 ops, 3 backends, 6 targets) + **probes de execução** (semântica observável real). **Documentação pura — zero mudança de código** (sem colisão com REFACTOR-500). **ESCRITO**: language-reference/{README,lexical-structure,grammar,syntax,types,type-system,expressions,statements,functions,closures,classes,modules,semantics,specification-status}.md + compiler-architecture.md + specification-gaps.md (SG-001..020 + divergências target C1-C8 + docs desatualizados E1-E3). README+architecture.md atualizados (linguagem≠compilador≠target). **Prova zero-regressão: suíte16 969/0/3-skip, BUILD SUCCESS** (idêntica à pré-LANG-SPEC). Achados que viram candidatos a bug/teste (NÃO corrigidos — decisão de design, regra 6): SG-008 (Int?==null NPE), SG-006 (short-circuit JS desligado), SG-009 (subtipagem não checada no type checker), SG-010 (val não-imutável), SG-001 (fun/fn compilam mas corpus diz que não). |
...
| **WEB002-Native residual** — `method()`/`path()`/`query()`/`header()`/`param()` + path-param `:id` + `status()`/`headerSet()` no asm | `FEITO` | agente-planning | `planning-future` | `NativeWebRuntime.java` (módulo não-gigante) | 05/09 4 commits (`bcb5575`..`5b1d650`): contexto de request (method/path/query + split query-less na rota), `header()` case-insensitive, `param()` com match por segmentos (`:id`, 16 slots), `status()`/`headerSet()` com status line dinâmica + headers. `KofWebNativeE2ETest` 8/8. **Residual:** TLS (`listenSecure`), ws/sse, keep-alive. |
| **TIER 6-12 super sessão** — mapear/implementar o que o REFACTOR-500 não bloquear | `BLOQUEADO (registrado)` | agente-planning | `planning-future` | `docs/future/blockers/BLOCKERS.md`, `docs/future/NAMESPACE-RUNBOOK.md`, `docs/status.md`, `docs/roadmap.md` | 05/09: coordenação atualizada + `BLOCKERS.md` TIER 6-12 detalhado + `NAMESPACE-RUNBOOK.md` (padrão de 8 passos p/ adicionar namespace stdlib, verificado). **Condição de parada (lane)**: todo tier exige novos namespaces → dispatch nos gigantes `CompilerDriver`/`SemanticAnalyzer`/`JvmRuntime`/`NativeRuntime`, todos `EM CURSO` no REFACTOR-500 por outros agentes. Nenhum código de tier é implementável sem colidir. Retomada: ordem em `BLOCKERS.md`. |

## Concluídos recentemente

| Gap/Item | Estado | Dono | Data | Prova |
|---|---|---|---|---|
| **MERGE beta-0.3.0 → planning-future** — refactor integrado + FFI/web/http reconciliados | `FEITO` | agente-planning | 06/09 | commits `5859071`(merge)+`9485720`(extern parser)+`9c53be7`(FFI/web context). FFI re-aplicado na arquitetura modularizada: resolução de `extern` no `BuiltinCallTyper` (nunca SEM015), lowering `kof_ffi_*` no `ExpressionStaticCallLowerer`, `externSignatures`/`isExternBound` package-private no `CompilerDriver`, `desugarEntity` de volta ao fluxo (`CompilerDesugar` moveu tests/application). Web runtime: `NativeWebRequestContext` + `NativeWebResponseContext`. Suíte **971/0** verde. VERSION guard intacto. |
...
| **docs/future finalizado** — docs implementados movidos para `docs/` (decompiler/differential/legacy-ir/legacy-migration/translator) + tradução dos `.md` para pt-BR | `FEITO` | agente-planning | 05/09 | `git mv` + status `EM DESENVOLVIMENTO`; `CODE_OF_CONDUCT`/`AGENTS` com filosofia Mel Santos |
...
|---|---|---|---|
| **HTTP003** — kof.http Native cauda | baixa | `https` + DNS real + `timeout/retry/circuit` (knobs reais) no Native | 05/09: **timeout ✅** (`8b66f7a`, SO_RCVTIMEO), **retry ✅ + circuit ✅** (`821d229`, loop de tentativas + breaker 30s). Restante: `https`/DNS real (bloqueado: sem libtls/libc no asm puro) |
