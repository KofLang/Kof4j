...
**EM CURSO (09/09, lane spec-gaps — 2ª rodada de decisões do maintainer):** SG-002 FEITO (tokens mortos removidos do Lexer: TILDE/COLON_COLON/ELLIPSIS/DOUBLE_ARROW/PIPE_LINE/UNDERSCORE + keywords sealed/permits — enum TokenType limpo; `=>`/`::`/`|>` agora lexam como chars simples e falham no parse, `~`/`sealed` dão LEX005; prova `deadTokensGiveCleanLexerError` 5 casos). **FILA (decisões verbatim do maintainer):** SG-011B (sobrecarga top-level homônima → erro SEM claro), SG-008 (é bug: `Int?==null` sem unbox, null safety não pode dar NPE + known-bugs), SG-020 (validar provas faltantes + atualizar docs do modelo), SG-005 (breaking change: deref de T? sem narrowing → erro, testes corrigidos junto), SG-009 (checar tipagem: subtipagem nominal em isAssignable). Suíte: 1236 run / 0 falhas MINHAS / 13 pré-existentes (12 + KofRandomTest de outra lane: export JS kofRandomInt pendente).
