...
| **LANG-SPEC** — Language Reference Manual: separar linguagem Kof do compilador (auditoria + spec formal extraída do código, sem mudar comportamento) | `EM CURSO` | agente-spec | `beta-0.3.0` | `docs/language-reference/*` (novo), `docs/compiler-architecture.md` (novo), `docs/specification-gaps.md` (novo), `README.md`, `docs/architecture.md` | 06/09: auditoria completa FEITA (lexer 477L, parser 1975L, AST 39 nós, SemanticAnalyzer, Type 8 records, IR 30 ops, 3 backends, 6 targets). **Documentação pura — NÃO toca código** (sem colisão com REFACTOR-500 F7/F6; referências por método, não só linha). Specs: lexical-structure, grammar (EBNF extraído do recursive descent), syntax, types, type-system, expressions, statements, functions, closures, classes, modules, semantics, specification-status + compiler-architecture + specification-gaps (SG-00x). Regra: extrair do código, nunca inventar; não-especificado = UNSPECIFIED. |
